...
class AbsModel: Caretable, ObservableObject, ArrowListener {
    ...
    func getData() -> ExpressionItemData {
        return AbsData(innerPartData: innerPartModel.getData(), id: self.id)
    }

    func replicate(_ expressionItemData: ExpressionItemData) {
        if let absData = expressionItemData as? AbsData {
            innerPartModel.replicate(absData.innerPartData)
            self.id = absData.id
        } else {
            fatalError("Not AbsData: \(expressionItemData)")
        }
    }
